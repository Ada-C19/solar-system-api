...
def create_moon(planet_id):
    ...
    return make_response(f"Moon {new_moon.name} successfully created for Planet {planet.name}", 201)

# GET ALL MOONS BY PLANET ENDPOINT
@planets_bp.route("/<planet_id>/moons", methods = ["GET"])
def handle_moons_from_planet(planet_id):
    planet = validate_model(planet_id)
